-- 0154: Typeform lead tier (boss ask 2026-08-19, analysis same day).
--
-- The active DC short forms carry exactly two scoring answers: the can-pay
-- qualify question (tf_qualified, 0133) and "What is your experience level
-- with AI?". Cross-tabbed on the 1,130 matched cohort leads the two combine
-- into a real pre-call score — close rates ran pay-yes+experienced 8.3%
-- down to pay-no+dabbler 0.8% (10×). This migration:
--
--   1. dc_ads_lead_facts.tf_experience — the lead's normalized experience
--      answer ('dabbler' | 'intermediate' | 'pro', null = not answered).
--   2. refresh_dc_ads_facts() stamps it in tf_resp/tf_match (the ONE home
--      of Typeform matching). The answer is recognized by its distinctive
--      choice labels — identical copy across every DC form to date — so a
--      new SheSells-style form works with zero registry config; unknown
--      copy falls to null (tier renders '—', never wrong).
--   3. dc_ads_lead_roster() returns 'tier' (A/B/C/D, null) derived beside
--      qualState: A = qualified + experienced (intermediate|pro), B =
--      qualified + dabbler, C = non-qualified + experienced, D =
--      non-qualified + dabbler; null unless BOTH answers are present.

-- 1. the column ------------------------------------------------------------

alter table dc_ads_lead_facts add column if not exists tf_experience text;

comment on column dc_ads_lead_facts.tf_experience is
  '0154: normalized "What is your experience level with AI?" answer from the lead''s matched Typeform response — dabbler | intermediate | pro; null = question not answered / no matched response / unrecognized copy.';

-- 2. refresh_dc_ads_facts() — 0148 definition + tf_experience ---------------
...
begin
  ...
  insert into dc_ads_lead_facts (
    ...
    source_kind, funnel_label, typeform_id, lp_slug, tf_qualified, tf_answered,
    tf_experience, updated_at)
  with leads_base as (
    select cl.close_id,
  ...
  -- experience (0154) = the normalized AI-experience answer, recognized by
  -- its choice labels (identical copy on every DC form to date).
  lp_rules as (
  ...
  tf_resp as (
    select r.form_id, r.submitted_at,
      ...
      (select case
         when a->'choice'->>'label' ilike '%dabbled%' then 'dabbler'
         when a->'choice'->>'label' ilike '%used it a good amount%' then 'intermediate'
         when a->'choice'->>'label' ilike '%experienced ai pro%' then 'pro'
       end
       from jsonb_array_elements(r.answers) a
       where a->>'type' = 'choice'
         and (a->'choice'->>'label' ilike '%dabbled%'
           or a->'choice'->>'label' ilike '%used it a good amount%'
           or a->'choice'->>'label' ilike '%experienced ai pro%')
       limit 1) as experience
  ...
  tf_match as (
    select distinct on (m.close_id) m.close_id, m.qualified, m.answered, m.form_id,
      m.experience
    from (
      select lp.close_id, r.submitted_at, r.qualified, r.answered, r.form_id, r.experience
      from lead_phones lp
      join tf_resp r on r.p10 <> '' and r.p10 = lp.p10
      union all
      select le.close_id, r.submitted_at, r.qualified, r.answered, r.form_id, r.experience
      from lead_emails le
...
select coalesce(jsonb_agg(jsonb_build_object(
  ...
  -- Typeform lead tier (0154): can-pay × AI-experience, the only pre-call
  -- score the funnel yields. Needs BOTH answers — else null ('—').
  'tier',      case
                 when f.tf_experience is null or not coalesce(f.tf_answered, false) then null
                 when coalesce(f.tf_qualified, false)
                      and f.tf_experience in ('intermediate', 'pro') then 'A'
                 when coalesce(f.tf_qualified, false) then 'B'
                 when f.tf_experience in ('intermediate', 'pro') then 'C'
                 else 'D'
               end,
  'connected', f.call90,
...
comment on function dc_ads_lead_roster(timestamptz, timestamptz, text, text, text, text, text, text) is
  'DC ads page: the embedded lead roster — identity + landing page + adId (0147) + dials + firstDial (0144) + smsOut/units (0145) + stage flags per cohort lead. The ONE per-lead read behind the page''s speed boxes, lead list, AND the per-ad speed block (grouped by adId in TS). qualState (0144): qualified / unqualified / partial. tier (0154): A/B/C/D from tf_qualified × tf_experience (null unless both answered) — historical close rates A≈8% … D≈1%. Since 0140 connected = call ≥90s only. Newest opt-in first. Facets AND together like the sibling RPCs.';
