-- 0132_dc_landing_pages.sql
--
-- The DC landing-page REGISTRY — the URL-keyed identity behind the DC Ads
-- page's landing-page dropdown (Drake 2026-08-13: LPs are named by their
-- actual URLs, shortened — 'join/training', 'go' — like the Hub's LP labels).
--
-- Why URL-keyed instead of the Close funnel_name (0130/0131's funnel_label):
-- the label rides Close's tagging, which cross-tags a handful of leads
-- ('Closer Funnel' on DC-campaign leads), and it can't be derived for a brand
-- new campaign until Close traffic flows. The ad creative's destination URL is
-- known the moment the campaign is detected, aligns spend scoping and facts
-- scoping on the SAME campaign set, and auto-extends: new funnel → new
-- destination URL → registry row auto-created by ingestion (auto_created=true,
-- label = short URL), videos auto-attached by the Wistia embed-location scan.
-- funnel_label stays on campaigns/facts as display metadata for the Paths
-- strip; it is no longer the filter key.
--
-- URL normalization (shared with ingestion/meta_ads/leads_parser.py):
-- lowercase host, strip scheme + query + fragment + trailing slash. So
-- 'https://go.digitalcollege.ai/?x=1' → 'go.digitalcollege.ai'.
--
-- Facts gain lp_slug, stamped at refresh from the campaign registry
-- (instant-form rows get the pseudo-slug 'instant-form' so the dropdown can
-- select the legacy path without special-case SQL). The four page RPCs gain
-- p_lp_slug; p_funnel_label is KEPT and still honored so the currently
-- deployed UI keeps working during the DB→deploy window — retire it once
-- nothing sends it.

-- ----------------------------------------------------------------- registry --

create table dc_landing_pages (
  slug text primary key,
  label text not null,
  lp_url text not null unique,
  page_urls text[] not null default '{}',
  typeform_id text,
  vsl jsonb not null default '[]'::jsonb,
  confirm_video_hashed_id text,
...
comment on table dc_landing_pages is
  'DC (Digital College) landing-page registry — the URL-keyed identity behind the DC Ads page''s landing-page dropdown. One row per ad-destination landing page on digitalcollege.ai. Auto-created by ingestion when a campaign''s creatives point at an unseen URL (auto_created=true, label = short URL); typeform_id auto-resolved from lead attribution; vsl auto-attached by the Wistia embed-location scan. DELIBERATELY separate from landing_pages (the high-ticket registry): that table''s form set defines HT funnel membership, so DC forms must never be added there. See docs/schema/dc_landing_pages.md.';
comment on column dc_landing_pages.label is
  'Short display name derived from the URL — subdomain + path with the shared digitalcollege.ai domain stripped (''join/training'', ''go''). Editable.';
comment on column dc_landing_pages.lp_url is
  'The normalized ad-destination URL (host+path, no scheme/query/trailing slash). THE join key from dc_ads_campaigns.destination_url.';
comment on column dc_landing_pages.page_urls is
  'Additional normalized funnel-page URLs belonging to this LP''s funnel (e.g. Luke''s VSL page join.digitalcollege.ai/t-2, reached after the opt-in). The Wistia embed scan matches videos against lp_url AND these.';
comment on column dc_landing_pages.vsl is
  'The video(s) shown in this LP''s funnel: [{"hashedId", "label"}] — same shape as landing_pages.vsl. Auto-appended by the Wistia embed-location scan; a video can appear on several LPs (DC_VSL_Thank you_v2 plays on both funnels today), so metrics are per-video across its embeds.';
comment on column dc_landing_pages.auto_created is
  'true = created by the ingestion resolver from an unseen destination URL. Rename the label / attach assets in place; the resolver never overwrites curated fields.';

create trigger dc_landing_pages_set_updated_at
  before update on dc_landing_pages
  for each row execute function set_updated_at();

alter table dc_landing_pages enable row level security;

-- Seeds — verified live 2026-08-13: campaign destinations from dc_ads_campaigns
-- (creative scan), video placement from Wistia's embed-location events
-- (DC_VSL_Thank you_v2 plays on join/training AND on Luke's /t-2 page),
-- typeforms from 0130 + typeform_forms titles.
insert into dc_landing_pages
  (slug, label, lp_url, page_urls, typeform_id, vsl, sort_order)
values
  ('join-training', 'join/training', 'join.digitalcollege.ai/training', '{}',
   'fEQEMEyR', '[{"hashedId": "12vs8bahck", "label": "DC_VSL_Thank you_v2"}]', 10),
  ('go', 'go', 'go.digitalcollege.ai', '{join.digitalcollege.ai/t-2}',
   'mKtmTD1H', '[{"hashedId": "12vs8bahck", "label": "DC_VSL_Thank you_v2"}]', 20);

-- -------------------------------------------------------- campaigns.lp_slug --

alter table dc_ads_campaigns
  add column lp_slug text references dc_landing_pages(slug);

comment on column dc_ads_campaigns.lp_slug is
  'The landing page this campaign drives to (normalized destination_url → dc_landing_pages.lp_url). Maintained by the ingestion resolver; null for instant_form campaigns (no landing page).';

update dc_ads_campaigns set lp_slug = 'join-training'
where destination_url = 'https://join.digitalcollege.ai/training';
update dc_ads_campaigns set lp_slug = 'go'
where destination_url = 'https://go.digitalcollege.ai/';

-- ------------------------------------------------------------ facts.lp_slug --

alter table dc_ads_lead_facts
  add column lp_slug text;

comment on column dc_ads_lead_facts.lp_slug is
  'The lead''s landing page (campaign registry lp_slug at refresh); ''instant-form'' pseudo-slug for instant_form leads so the dropdown can select the legacy path. The page''s landing-page facet key (p_lp_slug).';

create index dc_ads_lead_facts_lp_slug_idx on dc_ads_lead_facts (lp_slug);

-- ------------------------------------------------------------------ refresh --
-- Same body as 0130 plus the lp_slug stamp.
...
begin
  ...
  insert into dc_ads_lead_facts (
    ...
    source_kind, funnel_label, typeform_id, lp_slug, updated_at)
  with leads as (
    select cl.close_id,
...
comment on function refresh_dc_ads_facts() is
  'Rebuilds dc_ads_lead_facts (delete + insert, one transaction). Membership: close_leads on a campaign in dc_ads_campaigns (active), not excluded — BOTH acquisition paths. 0132 stamps lp_slug (campaign registry lp_slug; ''instant-form'' pseudo-slug for the legacy path).';

-- ------------------------------------------------------------------- facets --
-- The four page RPCs gain p_lp_slug (the landing-page dropdown's key).
-- p_funnel_label is kept AND still honored so the currently deployed UI keeps
-- working while this rolls out; the UI switches to p_lp_slug in the same
-- release and p_funnel_label can be retired in a later migration.

drop function dc_ads_funnel(timestamptz, timestamptz, text, text, text, text, text);
...
comment on function dc_ads_funnel(timestamptz, timestamptz, text, text, text, text, text, text) is
  'DC ads page: cohort funnel + speed-to-dial + time-of-day over dc_ads_lead_facts, anchored at the opt-in. Optional facets AND together: cascade entity (deepest only), p_form_id (0128), p_lp_slug (landing page, 0132; ''instant-form'' = legacy path). p_funnel_label is a deprecated alias facet kept for rollout compatibility.';

drop function dc_ads_funnel_by_rep(timestamptz, timestamptz, text, text, text, text, text);
...
comment on function dc_ads_funnel_by_rep(timestamptz, timestamptz, text, text, text, text, text, text) is
  'DC ads page: per-rep dials/connections/closes/cash, ACTIVITY-scoped to [p_start, p_end). Optional facets AND together: cascade entity, p_form_id (0128), p_lp_slug (0132). p_funnel_label is a deprecated alias facet kept for rollout compatibility.';

drop function dc_ads_daily(date, int, text, text, text, text, text);
...
comment on function dc_ads_daily(date, int, text, text, text, text, text, text) is
  'DC ads page: the last-N-ET-days cohort strip (opt-ins that day + lifetime progression + dials received). Optional facets AND together: cascade entity, p_form_id (0128), p_lp_slug (0132). p_funnel_label is a deprecated alias facet kept for rollout compatibility.';

drop function dc_ads_speed_cohort(timestamptz, timestamptz, text, text, text, text, text);
...
comment on function dc_ads_speed_cohort(timestamptz, timestamptz, text, text, text, text, text, text) is
  'DC ads page: per-lead timing/effort facts for the speed-to-lead boxes (business-hours math runs in TS). Optional facets AND together: cascade entity, p_form_id (0128), p_lp_slug (0132). p_funnel_label is a deprecated alias facet kept for rollout compatibility.';

-- Stamp lp_slug on the existing facts immediately.
