-- 0138_dc_ads_campaign_multi_lp.sql
--
-- Boss notes 2026-08-14: the campaign↔landing-page link becomes the page's
-- source of truth, and a campaign can carry SEVERAL landing pages (split
-- tests drive one campaign's traffic to two pages).
--
-- 1. dc_ads_campaigns gains lp_slugs text[] — every landing page the campaign
--    drives to. lp_slug stays as the PRIMARY (first) page: existing spend
--    scoping and single-LP facts stamping keep working, and the resolver
--    keeps maintaining it.
--
-- 2. Per-lead landing-page attribution for split-test campaigns: the campaign
--    alone can't say which page a lead came through, but the lead's OWN
--    Typeform submission can (each page embeds its own form; matched by
--    phone/email since 0133). refresh_dc_ads_facts now stamps:
--      instant_form                → 'instant-form'
--      campaign with ≤1 LP         → the campaign's lp_slug (campaign truth)
--      campaign with >1 LP         → the LP owning the lead's matched
--                                    Typeform, falling back to the primary
--    The TS layer then corroborates every "All landing pages" surface
--    (dropdown options + the Ads & LP summary) against ACTIVE campaigns'
--    lp_slugs — a landing page no active campaign drives to disappears from
--    the page (boss note 3).

alter table dc_ads_campaigns
  add column lp_slugs text[] not null default '{}';

comment on column dc_ads_campaigns.lp_slugs is
  'EVERY landing page this campaign drives to (dc_landing_pages.slug) — more than one when split-testing. lp_slug stays the primary (first) entry; the DC Setup Campaigns editor maintains both. The page''s landing-page dropdown and Ads & LP summary show exactly the union of ACTIVE campaigns'' lp_slugs.';

update dc_ads_campaigns set lp_slugs = array[lp_slug] where lp_slug is not null;

-- ------------------------------------------------------------------ refresh --
-- Same body as 0133 except: tf_match also carries WHICH form matched, and the
-- leads CTE resolves lp_slug through the split-test rule above.
