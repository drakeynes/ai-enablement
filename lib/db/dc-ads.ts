...
// One call's transcript-export section (0156): the review verdict + the
// conversation as "**Speaker N:** …" turns, built in SQL (the diarized
// words never leave Postgres). Consumed by the DC Calls Export button's
// route handler (app/(authenticated)/sales-dashboard/dc-ads/export).
export type DcAdsTranscriptSection = {
  callId: string
  at: string
  leadName: string
  repName: string | null
...
// Every exportable dc_ads call id, oldest first — pages past the REST
// max-rows cap so "Export all" survives cohort growth.
export async function getDcAdsTranscriptExportIds(): Promise<string[]> {
  const sb = createAdminClient()
  const ids: string[] = []
  const PAGE = 1000
  for (let from = 0; ; from += PAGE) {
    const { data, error } = await sb
      .rpc('dc_ads_transcript_export_ids' as never)
      .range(from, from + PAGE - 1)
    if (error) throw new Error(`dc_ads_transcript_export_ids RPC failed: ${error.message}`)
    const page = (data ?? []) as unknown as string[]
    ids.push(...page)
    if (page.length < PAGE) return ids
  }
}

// Sections for a list of call ids, in the given order's activity order.
// Chunked ~40 ids/RPC — one chunk reconstructs ~60k diarized words, well
// inside PostgREST's statement budget (measured ~0.8s).
export async function getDcAdsTranscriptSections(
  callIds: string[],
): Promise<DcAdsTranscriptSection[]> {
  const sb = createAdminClient()
  const CHUNK = 40
  const out: DcAdsTranscriptSection[] = []
  for (let i = 0; i < callIds.length; i += CHUNK) {
    const { data, error } = await sb.rpc('dc_ads_transcript_export' as never, {
      p_call_ids: callIds.slice(i, i + CHUNK),
    } as never)
    if (error) throw new Error(`dc_ads_transcript_export RPC failed: ${error.message}`)
    for (const r of (data ?? []) as unknown as Array<Record<string, unknown>>) {
      out.push({
        ...
        whyNotClosed: (r.why_not_closed as string) ?? null,
        repGap: (r.rep_gap as string) ?? null,
        archetype: (r.archetype as string) ?? null,
        leadScore: r.lead_score as number,
        intent: r.intent as number,
        offerUnderstanding: r.offer_understanding as number,
        repScore: r.rep_score as number,
        turns: (r.turns as string) ?? null,
      })
    }
  }
  return out
