...
- **M5 master sheet reconcile + completeness (cleanup pass, 2026-05-04):** `scripts/cleanup_master_sheet_reconcile.py` — two-phase USA + AUS CSV diff + tiered apply. First run landed 95 explicit DB writes touching ~70 unique clients: 36 status flips, 32 csm_standing flips (4 cascade-redundant skipped), 22 primary_csm reassignments, 13 trustpilot flips, 8 handover notes. **Delta re-run** against the canonical CSVs at `data/master_sheet/master-sheet-05-04/` landed 24 additional primary_csm reverts (cascade had reassigned negative-status clients to Scott Chasing during the prior apply; CSV still had the original real CSM). **Completeness pass** (`scripts/cleanup_master_sheet_completeness.py`) then autocreated 8 unmatched clients (4 N/A statuses → 'churned' with `metadata.original_master_sheet_status='N/A'` for forensics) and filled NULL gaps in crucial fields: 180 country (USA/AUS by tab), 180 start_date, 92 phone, 1 slack_user_id, 29 slack_channels inserts/relinks. End state: 203 non-archived clients (was 195); reconcile re-dry-run shows 0 Tier 1 changes — Gregory exactly matches CSV. All writes attributed to Gregory Bot UUID with notes `cleanup:m5_master_sheet_reconcile` and `cleanup:m5_completeness` for SQL audit (the two notes are SQL-joinable to attribute every cleanup-driven row to its pass). Tier 2 + Tier 3 ambiguities live in `docs/data/m5_cleanup_scott_notes.md` for Scott's onboarding meeting. Both scripts idempotent on re-run; cleanup is the canonical CSV-vs-Gregory reconciler (`import_master_sheet.py` is the build-time seed, not the steady-state tool).
...
**M5.5 + M5.6 + Path 2 outbound + M5 cleanup reconcile closed.** M5.5 (filter bar — 2026-05-03). M5.6 (status cascade + Scott Chasing + accountability/NPS toggles + same-day hotfix — 2026-05-04). Path 2 outbound roster (`api/accountability_roster.py` — daily-pull GET, 2026-05-04). M5 cleanup reconcile (`scripts/cleanup_master_sheet_reconcile.py` — Tier 1 applied 2026-05-04, 95 writes / 70 clients; Tier 2 + Tier 3 in `docs/data/m5_cleanup_scott_notes.md` for Scott meeting). Path 2's framing was reshaped by the Make.com walkthrough with Zain to a daily-pull GET (not the event-driven UPDATE listener previously sketched). See `docs/agents/gregory.md` § Build log for full close-outs.
...
3. **Cleanup follow-ups from the M5 reconcile + completeness (manual, no code).** Tier 2 + Tier 3 items from `docs/data/m5_cleanup_scott_notes.md` to walk through with Scott during onboarding (post-completeness pass — `unmatched` is now zero): 18 remaining Tier 2 items (mostly slack_user_id ambiguities + email mismatches that can't auto-fill), 15 "Owing Money" standings (CSM annotation Scott mashed into the Standing column — not a Gregory standing value), 2 email mismatches (per M5.4 backfill runbook — alternate_emails triage), 4 Aleks-owned reassignments (M4 Chunk C carry-over), 67 NPS Standing CSV-vs-Gregory differences (informational; Path 1 owns the column). Plus toggle re-activation for the 2 positive-transition clients from the prior apply (Marcus Miller, Allison Jayme Boeshans) — cascade is off-only by design (M5.6); positive transitions don't auto-revert toggles. Plus the 4 N/A-as-churned autocreates (Vaishali Adla, Scott Stauffenberg, Clyde Vinson, Rachelle Hernandez) for Scott's eyeball — discoverable via `WHERE metadata->>'original_master_sheet_status' = 'N/A'`.
