...
1. **Path 2 outbound writeback architecture (TOP PRIORITY).** Make.com walkthrough with Zain happens during the next session — Zain will be active and working. Everything must be in place to scope and start architecture during/after that conversation. Pre-meeting: a scoping doc / question list ready for the walkthrough. Path 1 (M5.4) is one-way Airtable → Gregory; Path 2 is the inverse — dashboard edits (cascade-fired or CSM-manual) propagate back to upstream Airtable. Architecture narrowed by M5.6: the `accountability_enabled` + `nps_enabled` columns now exist; Path 2 listens on UPDATE of those specific columns via a Supabase Database Webhook (or a parallel trigger). Both cascade and manual paths land in the same UPDATE so the receiver doesn't need to distinguish them.
