...
**Batch A — CSM accountability visibility (in progress).** Two sub-items shipped 2026-05-05 (M6.1): per-call CS Slack summary auto-post on every Fathom client call delivery + daily 7am EST accountability notification cron posting per-CSM missing-submitter lists. Remaining sub-items: missed/unrecorded call detection (Google Calendar diff against `calls`; depends on Drake's Fathom team-settings sweep closing the duplicate-recording bug first) and the call-tagging dashboard (depends on CSM ops adopting a tagging convention). See `docs/future-ideas.md` § Batch A.
...
1. **Batch A — CSM accountability visibility (in progress).** Per-call CS summary + daily accountability notification shipped 2026-05-05 (M6.1). Remaining sub-items: missed/unrecorded call detection (gated on Drake's Fathom team-settings sweep), call-tagging dashboard (gated on CSM ops adoption of a tagging convention). See `docs/future-ideas.md` § Batch A.
