...
1. **M1.2.5 — Deploy `api/fathom_backfill.py` + first manual cron run.** M1.2 (2026-04-27) built and locally verified the backfill handler (auth-only paths green; Fathom-against-cloud paths skipped because `FATHOM_API_KEY` isn't in `.env.local`). M1.2.5 is the deploy + activation step: (a) generate `BACKFILL_AUTH_TOKEN` via `openssl rand -hex 32`, save to Bitwarden; (b) generate `FATHOM_API_KEY` from the Fathom team account, save to Bitwarden; (c) set `FATHOM_API_KEY`, `BACKFILL_AUTH_TOKEN`, and `CRON_SECRET` (same value as `BACKFILL_AUTH_TOKEN`) on Vercel Production scope; (d) push the M1.2 commit (handler + `vercel.json` cron entry); (e) manually trigger the first sweep via `curl -i -X POST -H "Authorization: Bearer <token>" https://.../api/fathom_backfill` and verify the summary JSON shows `meetings_seen > 0`, `already_present > 0` (proves idempotency against F1.4 backlog), and any `ingested` count covers post-registration weekend calls; (f) verify downstream rows landed for ingested external_ids. Full step-by-step in `docs/runbooks/fathom_webhook.md` § Backfill cron / Deploy. The first sweep also serves as M1.1's final verification — it'll surface any post-fix-registration weekend calls AND any ingest-pipeline issues that haven't been exercised against real Fathom data yet.
