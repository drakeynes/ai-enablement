...
- **M5 master sheet reconcile (cleanup pass, 2026-05-04):** `scripts/cleanup_master_sheet_reconcile.py` — two-phase USA + AUS CSV diff + tiered apply. First run landed 95 explicit DB writes touching ~70 unique clients: 36 status flips, 32 csm_standing flips (4 cascade-redundant skipped), 22 primary_csm reassignments (16 applied, 6 already matched), 13 trustpilot flips, 8 handover notes. All RPCs attributed to Gregory Bot UUID with `cleanup:m5_master_sheet_reconcile` note for SQL audit. 0 errors, 0 post-apply Bucket B mismatches — Gregory matches CSV across all Tier 1 fields. Tier 2 (44 items: 26 slack_channel coverage gaps, 15 Owing-Money standings, 2 email mismatches, 1 slack_user_id gap) + Tier 3 (63: 59 NPS standing differences, 4 Aleks-owned) live in `docs/data/m5_cleanup_scott_notes.md` for Scott's onboarding meeting. Idempotent on re-run; cleanup script is now the canonical CSV-vs-Gregory reconciler (replaces ad-hoc dashboard walkthrough; `import_master_sheet.py` is the build-time seed, not the steady-state tool).
...
3. **Cleanup follow-ups from the M5 reconcile (manual, no code).** Tier 2 + Tier 3 items from `docs/data/m5_cleanup_scott_notes.md` to walk through with Scott during onboarding: 26 slack_channel coverage gaps (clients with no slack_channels row but CSV has one), 15 "Owing Money" standings (CSM annotation Scott mashed into the Standing column — not a Gregory standing value), 2 email mismatches (per M5.4 backfill runbook — alternate_emails triage), 4 Aleks-owned reassignments (M4 Chunk C carry-over), 59 NPS Standing CSV-vs-Gregory differences (informational; Path 1 owns the column), 3 unmatched-with-email rows (Anthony Huang / Matthew Gibson / Melvin Dayal — likely new clients to create), 5 unmatched-without-email rows (Scott decides per-client). Plus toggle re-activation for the 2 positive-transition clients from today's apply (Marcus Miller, Allison Jayme Boeshans) — cascade is off-only by design (M5.6); positive transitions don't auto-revert toggles.
