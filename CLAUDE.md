...
│   └── crm/                    # (planned)
├── api/                        # Vercel Python serverless functions (5 deployed)
...
**Today's session (2026-05-03) shipped:**
...
1. **M5.5 — comprehensive filtering.** Spec defined: 5 active filter dropdowns (Status, Primary CSM, CSM Standing, NPS Standing, Trustpilot) with multi-select + AND-across-fields / OR-within-field, plus 2 disabled placeholder dropdowns (Accountability, NPS toggle) signaling future state for Scott's onboarding. UX answers confirmed: A defaults (status pre-checked active+ghost+paused), all-unchecked = no filter, URL persistence comma-separated, sort orthogonal, all 7 dropdowns always-present (no "+ Add filter" bucket). Prompt drafting happens in next session — half-drafted conceptually, not in any file. Highest priority for Scott's adoption.

2. **M5 status cascade.** Scott Chasing column + automated `csm_standing` → `at_risk` transitions on negative status changes. Architecture pending; will define the column shape + cascade rules. Ties into priority #3 below.

3. **Trustpilot auto-correct on standing change** (Scott's Loom 2). When `csm_standing` transitions in certain directions, auto-flip `trustpilot_status` to a corresponding state. Concrete rule pending Loom 2 walkthrough. Implementation likely sits on top of #2's cascade infrastructure.

4. **Path 2 outbound writeback architecture.** Pending Make.com walkthrough with Zain Monday 2026-05-04. Path 1 is one-way (Airtable → Gregory); Path 2 is the inverse — dashboard edits propagate back to upstream Airtable / future master-sheet replacement.

5. **Inactivity flag** — end-of-week-2 monthly meeting count. Scope pending Scott's Loom 2.

6. **Australia/US country tagging.** Column already populated from master sheet; needs filter / display surfaces.

7. **CSM-edit lockdown** (Scott's Loom 1). Restrict who can edit which client fields. Boundary definition pending Loom 1.

8. **Email-mismatch cleanup from M5.4 backfill.** Jonathan Duran-Rojas + Luis Malo — manual triage per `docs/runbooks/backfill_nps_from_airtable.md` § "Failure modes." Drake handling over weekend.

9. **NPS score piping (V1.5).** Extend Path 1 to include score alongside segment. Path 1 segment-only is sufficient for Monday onboarding; score is a follow-up enhancement.
