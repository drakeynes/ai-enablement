...
**M5.5 + M5.6 closed.** M5.5 (filter bar — shipped + pushed 2026-05-03). M5.6 (status cascade + Scott Chasing + accountability/NPS toggles + same-day hotfix for three regressions surfaced by visual smoke — shipped 2026-05-04). The cascade infrastructure (BEFORE/AFTER triggers + Scott Chasing sentinel + GUC attribution + the new `is_csm` column gating dashboard dropdowns) is the foundation the next two priorities sit on top of. See `docs/agents/gregory.md` § Build log for full close-outs.
