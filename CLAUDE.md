...
- **Fathom webhook handler:** `api/fathom_events.py` exists and is locally verified across all 5 HTTP paths (happy / duplicate-replay / bad-signature / malformed-payload / ingest-failure) as of F2.4 (2026-04-24). **Not yet deployed to Vercel**; not yet registered with Fathom. Deploy + register happens in F2.5. Full architecture in `docs/architecture/fathom_webhook.md`. Requires env var `FATHOM_WEBHOOK_SECRET` on Vercel (set during F2.5). The handler calls into the F2.3-built `ingestion.fathom.webhook_adapter.record_from_webhook` → `ingestion.fathom.pipeline.ingest_call` chain — same chain that produces `call_summary` + `call_action_items` rows when the payload carries them.
...
1. **F2.5 — Deploy `api/fathom_events.py` to Vercel + register the production webhook.** The handler is built and locally tested (F2.4 ran all 5 paths — happy / duplicate / bad-signature / malformed / ingest-failure — against cloud and passed). F2.5 is the one-time ops work: (a) add `api/fathom_events.py` to `vercel.json`'s `functions` block with `maxDuration: 60`, (b) push to Vercel (existing Vercel project, same one hosting `api/slack_events.py`), (c) confirm GET `https://ai-enablement-sigma.vercel.app/api/fathom_events` returns the browser-friendly hint 200, (d) generate the production Fathom API key from Drake's Fathom team account, (e) register the webhook via `POST /webhooks` with `destination_url` = the Vercel URL, `triggered_for` = `["my_recordings","shared_team_recordings"]`, all four `include_*` flags true, (f) paste the returned `whsec_...` secret into Vercel env var `FATHOM_WEBHOOK_SECRET` and redeploy, (g) smoke-test with a short test meeting. Once the first real delivery lands, the 4 open F2.1 unknowns (webhook-id stability across retries, retry schedule, duplicate deliveries on re-summary, plan-tier confirmation) close empirically.
2. **F2.6 — Daily backfill cron `api/fathom_backfill.py`.** Per `docs/architecture/fathom_webhook.md` §f. Vercel Cron at 08:00 UTC, `GET /meetings?created_after=<last-received>&include_*=true` through cursor pagination, run each unseen call through the same adapter + pipeline. Written after F2.5 has proven the happy-path; the backfill is the safety net, not the first line of ingest.
