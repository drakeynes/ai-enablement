...
As of 2026-05-03 (M5.5 filter bar close-out, awaiting push):
...
**M5.5 closed 2026-05-03** (shipped + pushed; visual smoke through the auth-gated UI pending Drake's eyeball). Filter bar is a 9-dropdown row with 5 active multi-selects + 1 toggle + 3 disabled placeholders. Vocab module + reusable `MultiSelectDropdown` primitive in place. See `docs/agents/gregory.md` § Build log for the full close-out, including the deliberate `primary_csm_id` → `primary_csm` URL param rename.

1. **M5 status cascade.** Scott Chasing column + automated `csm_standing` → `at_risk` transitions on negative status changes. Architecture pending; will define the column shape + cascade rules. Ties into priority #2 below. The disabled "Accountability" placeholder in the M5.5 filter bar may absorb the Scott Chasing filter outright depending on cascade scope.

2. **Trustpilot auto-correct on standing change** (Scott's Loom 2). When `csm_standing` transitions in certain directions, auto-flip `trustpilot_status` to a corresponding state. Concrete rule pending Loom 2 walkthrough. Implementation likely sits on top of #1's cascade infrastructure.

3. **Path 2 outbound writeback architecture.** Pending Make.com walkthrough with Zain Monday 2026-05-04. Path 1 is one-way (Airtable → Gregory); Path 2 is the inverse — dashboard edits propagate back to upstream Airtable / future master-sheet replacement.

4. **Inactivity flag** — end-of-week-2 monthly meeting count. Scope pending Scott's Loom 2.

5. **Australia/US country tagging.** Column already populated from master sheet; needs filter / display surfaces. The disabled "Country" placeholder in the M5.5 filter bar wires up here once the column has dedicated semantics.

6. **CSM-edit lockdown** (Scott's Loom 1). Restrict who can edit which client fields. Boundary definition pending Loom 1.

7. **Email-mismatch cleanup from M5.4 backfill.** Jonathan Duran-Rojas + Luis Malo — manual triage per `docs/runbooks/backfill_nps_from_airtable.md` § "Failure modes." Drake handling over weekend.

8. **NPS score piping (V1.5).** Extend Path 1 to include score alongside segment. Path 1 segment-only is sufficient for Monday onboarding; score is a follow-up enhancement.
