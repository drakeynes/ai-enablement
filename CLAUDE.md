...
│   └── crm/                    # (planned)
├── api/                        # Vercel Python serverless functions (7 deployed)
...
**Batch A — CSM accountability visibility.** Top-priority Gregory V2 work. Sub-items are listed in `docs/future-ideas.md` § Batch A and include: per-call CS Slack summary, daily 7am EST accountability notification, missed/unrecorded call detection (with associated Fathom team-settings cleanup for the duplicate-recording bug), and the call-tagging dashboard. Active work decomposes into (a) ops-side Fathom team settings sweep (Drake), (b) Slack message infrastructure for cross-CSM channels, (c) Google Calendar diff for missed-call detection.
...
1. **Batch A — CSM accountability visibility.** Active. See `docs/future-ideas.md` § Batch A for sub-item descriptions and revisit triggers. First slice: confirm Drake's Fathom team-settings cleanup approach so the duplicate-recording bug closes before missed-call detection lands.
