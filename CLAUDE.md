...
**M5.5 + M5.6 closed.** M5.5 (filter bar — shipped + pushed 2026-05-03). M5.6 (status cascade — built + applied 2026-05-04, awaiting Drake's visual smoke before push). The cascade infrastructure (BEFORE/AFTER triggers + Scott Chasing sentinel + GUC attribution + the new `is_csm` column gating dashboard dropdowns) is the foundation the next two priorities sit on top of. See `docs/agents/gregory.md` § Build log for full close-outs.

1. **M5.7 — Trustpilot auto-correct on standing change** (Scott's Loom 2). When `csm_standing` transitions in certain directions (e.g. → `happy`), auto-flip `trustpilot_status` to a corresponding state (likely → `ask`). Concrete rule pending Loom 2 walkthrough. Implementation pattern is the same as M5.6: a trigger on `clients` gated on `csm_standing` transitions, mutating in-flight, GUC-attributed history. Ties into the existing M5.6 cascade infrastructure.

2. **Path 2 outbound writeback architecture.** Pending Make.com walkthrough with Zain (target Monday 2026-05-04). Path 1 is one-way (Airtable → Gregory); Path 2 is the inverse — dashboard edits propagate back to upstream Airtable / future master-sheet replacement. M5.6 shipped the `accountability_enabled` + `nps_enabled` columns the cascade owns; Path 2 narrows to "fire webhook on UPDATE of these specific columns" regardless of whether the change came from the cascade or a CSM manual toggle. Both code paths land in the same `UPDATE clients` so a Supabase webhook (or a parallel trigger) on UPDATE of those columns fires correctly for both.

3. **Inactivity flag** — end-of-week-2 monthly meeting count. Scope pending Scott's Loom 2.

4. **Australia/US country tagging.** Column already populated from master sheet; needs filter / display surfaces. The disabled "Country" placeholder in the M5.5 filter bar wires up here once the column has dedicated semantics.

5. **CSM-edit lockdown** (Scott's Loom 1). Restrict who can edit which client fields. Boundary definition pending Loom 1.

6. **Email-mismatch cleanup from M5.4 backfill.** Jonathan Duran-Rojas + Luis Malo — manual triage per `docs/runbooks/backfill_nps_from_airtable.md` § "Failure modes." Drake handling over weekend.

7. **NPS score piping (V1.5).** Extend Path 1 to include score alongside segment. Path 1 segment-only is sufficient for Monday onboarding; score is a follow-up enhancement.

8. **32-client primary_csm post-M5.6 cleanup decision.** 32 currently-CSM-owned negative-status clients (Lou 18 / Scott Wilson 13 / Nabeel 1) kept their existing primary_csm assignments through the M5.6 backfill. Drake decides whether to manually reassign any of them to Scott Chasing via the dashboard's swap-CSM dialog — going forward the cascade auto-reassigns on new negative transitions, so this is a one-time backlog choice.
