...
- **Fathom webhook handler:** `api/fathom_events.py` **deployed to Vercel** via commits `bc4cbbb..e9431da` (F2.5, 2026-04-24). `GET https://ai-enablement-sigma.vercel.app/api/fathom_events` returns the browser-friendly hint 200. Webhook registered with Fathom via their Settings UI; `FATHOM_WEBHOOK_SECRET` set in Vercel production env. **End-to-end live-delivery verification pending** — no real coaching call has finished Fathom's post-processing since registration, so no `webhook_deliveries` row has appeared yet. Full resume procedure in `docs/runbooks/fathom_webhook.md` § Resume from F2.5 pause. Previously locally verified across all 5 HTTP paths in F2.4 (happy / duplicate-replay / bad-signature / malformed-payload / ingest-failure). Full architecture in `docs/architecture/fathom_webhook.md`.
...
1. **F2.5 verification — confirm first real delivery lands cleanly.** F2.5 deployed + registered but paused before a real payload arrived. Lightweight resume: run the polling query in `docs/runbooks/fathom_webhook.md` § Resume (`select webhook_id, processing_status, received_at, call_external_id, processing_error from webhook_deliveries order by received_at desc limit 10`). If a row has landed and shows `processing_status='processed'` → verification done, mark the F2.5 followup RESOLVED. If nothing landed, either wait for organic team-call traffic or record a test meeting; Resume § lists the four diagnostics in order if nothing arrives within 15 min of a recorded call. Plan-tier (F2.1 unknown #4) is effectively resolved — UI registration succeeded without upgrade prompt; the other three unknowns (webhook-id stability across retries, retry schedule, duplicate deliveries on re-summary) remain formally open but defensively handled in the architecture.
2. **F2.6 — Daily backfill cron `api/fathom_backfill.py`.** Per `docs/architecture/fathom_webhook.md` §f. Vercel Cron at 08:00 UTC, `GET /meetings?created_after=<last-received>&include_*=true` through cursor pagination, run each unseen call through the same adapter + pipeline. Start after F2.5 verification confirms the happy path works; the cron is the safety net, not the first line of ingest. Critical once the webhook is trusted — without it, any delivery Fathom fails to retry for eventually falls off the retry window and the call is only recoverable via the TXT backlog runbook.
