...
**Non-goals:** Reorganization of the existing schema. The known design seam between `alerts` and `client_health_scores.factors.concerns[]` is deferred and tracked in `docs/followups.md` § "`alerts` vs. `client_health_scores.factors.concerns[]` — two-table redundancy" — concerns stay in jsonb for V1; will resolve when CSM Co-Pilot needs a single read source.
