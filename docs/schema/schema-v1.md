...
| `update_client_status_with_history(p_client_id uuid, p_new_status text, p_changed_by uuid default null, p_note text default null) → clients` | 0018 | Atomic `clients.status` update + `client_status_history` insert. Validates enum (`active`/`paused`/`ghost`/`churned`). Idempotent no-op when `current = new` (no history row written). Called by the Gregory dashboard from Section 1 (Identity & Contact). |
