...
- **What:** M1.2 added two new env vars: `FATHOM_API_KEY` (Fathom team-account API key, used by `api/fathom_backfill.py` to read `/meetings`) and `BACKFILL_AUTH_TOKEN` (random secret, used by Vercel Cron's `Authorization: Bearer ...` header). Neither has a documented rotation procedure today. The Fathom API key has the same constraint as the webhook secret — Fathom's API doesn't expose a rotate endpoint, only delete + recreate.
...
## F2.5 first-real-delivery verification — pending M1.2.5 cron run; root cause was Fathom UI viewport clipping

- **Update 2026-04-27 PM:** the silence had a real cause. M1.1 originally inferred "no team calls happened in the window" but Drake confirmed real coaching calls *did* happen over the weekend. **Root cause: Fathom's webhook registration UI has a viewport bug** — the verify/save button rendered below the fold with no scroll on Drake's display, so the form was filled but never successfully submitted. The webhook had been registered in name only — the registration object existed but Fathom hadn't sent any deliveries to it. Workaround: zoom out the browser before submitting. Webhook is now actually registered + verified live; pre-fix calls are gone (Fathom doesn't retry beyond its window).
- **Cloud state today (post-fix, post-registration-actually-working):** still zero `webhook_deliveries` rows, exact F1.4 baseline on every table. **Reason now is plausibly cadence** — registration only completed today, no team calls have finished Fathom post-processing since then. M1.2 (this session) built the cron backfill; M1.2.5 deploys it; the first real cron run will be the empirical proof that the full chain works end-to-end against real Fathom data.
- **Next action:** ship M1.2.5 (deploy + add `FATHOM_API_KEY` and `BACKFILL_AUTH_TOKEN` env vars to Vercel + trigger first manual cron run). The first sweep should produce a non-zero `ingested` count (recovers any post-registration weekend calls that fell in the cron's lookback window) AND a high `already_present` count (proves idempotency against the F1.4 backlog). When that lands cleanly, mark this followup RESOLVED.
- **Logged:** 2026-04-24 (F2.5 paused); 2026-04-27 (M1.1 zero-delivery check); **root cause identified:** 2026-04-27 (Fathom UI bug); **build complete:** 2026-04-27 (M1.2 cron backfill local-tested, awaiting M1.2.5 deploy).
