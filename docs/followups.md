...
## F2.5 first-real-delivery verification pending

- **What:** F2.5 (2026-04-24) deployed `api/fathom_events.py` to Vercel, set `FATHOM_WEBHOOK_SECRET` on the production env, and registered the webhook with Fathom (via UI, not API — so the webhook id wasn't captured; retrievable later via `GET /webhooks` if needed for rotation or teardown). GET endpoint verified live. **But no real delivery has been observed yet.** Cloud state at pause: exact F1.4 baseline (516 calls, 0 webhook_deliveries, 0 call_action_items) — nothing fired during the session because no team call finished Fathom's post-processing in that window.
- **Why it matters:** the "webhook is live" state is assumed from successful registration + 200 GET, but not confirmed by a real payload round-tripping. Signature verification against Fathom's real signer, adapter handling of a real payload shape, and the full ingest chain reporting `processing_status='processed'` have all been tested only against synthetic payloads + the local test harness. A first real delivery that lands `failed` or `malformed` would surface an unknown that the F2.1 doc read + F2.3/F2.4 unit/integration tests missed. Pilot rollout should wait on this verification.
- **Next action:** two paths — (a) passive: wait for organic team-call traffic; check `webhook_deliveries` periodically via the query in `docs/runbooks/fathom_webhook.md` § Resume. (b) active: record a ≥90s Fathom meeting, wait 3–5 min, check the same query. If nothing lands within 15 min, runbook's Resume § lists the four diagnostics in order (Fathom processing status → webhook scope → Vercel 401 logs → Fathom delivery history). Full runbook covers both paths. Mark resolved once the first `processing_status='processed'` row lands with downstream rows populated correctly.
- **Logged:** 2026-04-24 (F2.5 paused mid-session).
