...
## No archive button on client detail page

- **What:** the client detail page has no "Archive" affordance. Today's misclassified-client archive (Andy / Aman / Branden) required `scripts/archive_misclassified_clients.py` because the dashboard couldn't do it. The merge flow does archive (the source row gets `archived_at` set when merged into a target), but a standalone archive without merge isn't exposed.
- **Why it matters:** every misclassification archive becomes a script-run instead of a CSM dashboard action. Recurring pattern for Fathom false-positives, abandoned-pre-onboarding rows, and any other "this isn't actually a client" case. The archive operation itself is one UPDATE — keeping it script-only is over-cautious.
- **Next action:** add an Archive button to the client detail page Identity & Contact section (or a separate "Danger zone" footer like merge) wired through a Server Action that does the same `archived_at + metadata` pattern this script uses. Confirmation dialog mirroring the merge dialog. The metadata.archived_via key should accept a free-text string (e.g. "manual_dashboard_archive") so the audit trail distinguishes script-driven from dashboard-driven archives.
- **Logged:** 2026-05-05 (M5 misclassified-client archive — script existed because dashboard didn't).
