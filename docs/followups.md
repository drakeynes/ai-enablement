...
## Fathom webhook — delivery semantics live-test — ready for Drake to execute

- **What:** F2.1 identified four unknowns in Fathom's docs that need empirical confirmation: (1) `webhook-id` stability across retries, (2) retry count + backoff schedule on non-2xx responses, (3) whether summary regeneration fires a second `new-meeting-content-ready` for the same `recording_id`, (4) plan-tier gating on webhook registration. F2.2 set up the test infrastructure but could not execute the test because registering a webhook requires Drake's Fathom login and recording a test meeting requires his calendar. A **throwaway webhook.site URL** was created for the test: `https://webhook.site/fd90937a-30d4-4985-bd2d-7e483fb428f8` (expires 2026-05-01). The F2.2 session report includes the exact `curl` to register, the exact `curl` to flip webhook.site to return 500 for retry testing, and the exact `curl` to read captured deliveries back. Architecture in `docs/architecture/fathom_webhook.md` is built defensively so F2.3 can start before these land: dedup is double-layered (`webhook_deliveries` PK + `calls_source_external_id_key`), failure mode on 500 is documented, cron backfill catches any gap Fathom's retry doesn't close.
- **Why it matters:** architecture is robust to worst-case on all four, so F2.3 is NOT blocked. But the actual numbers sharpen operational expectations: (1) tells us whether the handler's PK dedup is primary or secondary defense, (2) tells us how long our Vercel deployment can be down before cron has to carry the load, (3) tells us whether `call_summary` metadata-sync is exercised naturally by Fathom's product behavior, (4) tells us whether the webhook path is available at all (fallback = cron-only every 30–60 min).
- **Next action:** Drake runs the F2.2 test runbook (in the session report). ~15 min of active time + meeting-recording latency (Fathom takes 2–5 min post-call to fire `new-meeting-content-ready`). When results land, update `docs/architecture/fathom_webhook.md` § Open Questions and close this followup.
- **Logged:** 2026-04-24 (F2.1 discovery); **prepped:** 2026-04-24 (F2.2 infra ready, waiting on Drake).
